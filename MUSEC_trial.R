### Code for Section 3 (Table 2 and Figure A.2.1): calculation of CIs for the 
### MUSEC trial reported by Bauer et al. and Zajicek et al.
...
s1_1 = 27    # Number of subjects with relief from muscle stiffness in CE arm
s0_1 = 12    # Number of subjects with relief from muscle stiffness in placebo
...
s1_2 = 42    # Number of subjects with relief from muscle stiffness in CE arm
s0_2 = 21    # Number of subjects with relief from muscle stiffness in placebo
...
N = 10^4   # Number of bootstrap replicates
...
# print(round(100*(CIs_raw[,2] - CIs_raw[,1])/(CIs_raw[1,2] - CIs_raw[1,1])-100,0))
...
CI_data = data.frame(point.estimates = point.estimates,
                     CI.lower = CIs[,1], CI.upper = CIs[,2])



CI_data$method = factor(CI_names, levels = rev(CI_names))


CI_data$linetype = c(rep("solid", 5), rep("dashed", 4))
# Plot with conditional line types for error bars
ggplot(CI_data, aes(y = method, x = point.estimates)) +
  geom_point() +
  geom_errorbar(aes(xmin = CI.lower, xmax = CI.upper, linetype = linetype), width = 0.2) +
  scale_linetype_manual(values = c("solid" = "solid", "dashed" = "dashed")) + # Ensure correct linetypes are used
  labs(x = "Estimate", y = "CI method") +
  xlim(c(0, 0.4)) +  guides(linetype = "none") 